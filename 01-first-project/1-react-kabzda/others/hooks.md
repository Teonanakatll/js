...
***                                               CreateSelector**
> 1. Для чего?
> Оптимизирует выборку данных из Redux-стора:
> Мемоизация — кеширует результат, пока не изменятся входные данные.
> Избегает лишних перерисовок компонентов.
> 
> 2. Как работает?
> Принимает:
> Массив зависимостей — другие селекторы или функции.
> Функцию-трансформер — вычисляет результат на основе зависимостей.
```js
...
const selectActiveUsers = createSelector(
  [selectUsers], // Зависимости
  (users) => users.filter(user => user.isActive) // Трансформер
);
```
> 3. Когда использовать?
> Дорогие вычисления (фильтрация, сортировка).
> Частые обновления стора, но редко меняющиеся данные.
> Вложенные данные, чтобы избежать лишних перерисовок.
> 
> 4. Что запомнить?
> 🔹 Возвращает новый селектор, который можно использовать в useSelector.
> 🔹 Пересчитывает результат только при изменении зависимостей.
> 🔹 Не создавайте селекторы внутри компонентов — они теряют мемоизацию.

```js
const activeUsers = useSelector(selectActiveUsers); // Автоматически мемоизировано
Итог: createSelector — это «умный» селектор для производительности.
```
